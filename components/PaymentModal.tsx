...
export const PaymentModal: React.FC<PaymentModalProps> = ({ inscripcion, onClose, onSuccess, pagoToEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#59A9B9] z-[100] flex items-center justify-center p-4 overflow-y-auto">
      {/* Container principal */}
      <div className="w-full max-w-[414px] flex flex-col gap-4 animate-in zoom-in-95 duration-300">

        {/* ÁREA DE CAPTURA - ESTILO TICKET */}
        {/* Este div (modalRef) es lo que sale en la foto. Fondo Teal + Card Blanca Centrada */}
        <div ref={modalRef} className="bg-[#59A9B9] p-6 flex flex-col items-center justify-center rounded-[20px]">
          <div className="bg-white w-full rounded-[32px] overflow-hidden shadow-2xl">
            ...
            {/* Branding en el ticket (Footer interna del ticket) */}
        ...
        {/* Acciones del Sistema - FUERA del modalRef para que no salgan en la foto */}
        <div className="px-[20px] py-[10px] w-full">
          {!isConfirmed ? (
            <button onClick={handleConfirmPayment} disabled={loading} className="w-full h-[42px] bg-white text-[#26475C] rounded-[16px] text-[14px] font-bold uppercase tracking-widest active:scale-95 transition-all shadow-xl">
